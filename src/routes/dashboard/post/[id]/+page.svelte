<script lang="ts">
	...
	const localReplies = $derived($activityService.filter((a: Activity) => a.parent_id === activityId));
...
<svelte:head>
	<title>{(mainPost?.payload as unknown as import('$lib/services/activity').PostPayload)?.content?.slice(0, 50) || 'Post'} | NeuroHub Hub</title>
</svelte:head>
		...
		<div class="mx-auto max-w-3xl">
			...
			<div class="mb-8 flex items-center gap-4">
				<button 
					...
					aria-label="Go back"
				>
					<svg class="h-5 w-5 text-content-dim group-hover:text-brand-orange" fill="none" viewBox="0 0 24 24" stroke="currentColor">
						<path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M10 19l-7-7m0 0l7-7m-7 7h18" />
					</svg>
				</button>
				<div>
					<h1 class="text-2xl font-black tracking-tight text-content">Post Detail</h1>
					<p class="text-xs font-bold text-content-dim uppercase tracking-wider">Conversation</p>
				</div>
			</div>

			{#if loading}
				<div class="space-y-6 animate-pulse">
					<div class="h-64 rounded-3xl bg-surface-dim/50 border border-stroke shadow-xl"></div>
					<div class="h-32 rounded-3xl bg-surface-dim/20 border border-dashed border-stroke"></div>
				</div>
			{:else if mainPost}
				<div in:fade={{ duration: 400 }}>
					<!-- Main Post Container -->
					<div class="mb-10 overflow-hidden rounded-3xl border border-stroke bg-surface-dim shadow-2xl">
						<ActivityItem activity={mainPost} />
					</div>

					<!-- Reply Section -->
					<div class="mb-12 pl-6 border-l-2 border-brand-orange/30 ml-6">
						<p class="mb-4 text-[10px] font-black tracking-[2px] text-zinc-500 uppercase">Post your reply</p>
					...
					<div class="space-y-8">
						...
						{#if localReplies.length === 0}
							<div class="py-16 text-center rounded-3xl border border-dashed border-stroke bg-surface-dim/20">
			...
			{:else}
				<div class="py-24 text-center rounded-3xl border border-stroke bg-surface-dim/20">
					<p class="text-content-dim italic">This post could not be found or was deleted.</p>
					<button onclick={() => history.back()} class="mt-4 text-brand-orange font-bold text-sm hover:underline">Go Back</button>
				</div>
			{/if}
		</div>
	</div>
