<script lang="ts">
	...
	import MessageChunk from './MessageChunk.svelte';
	...
	let topObserver = $state<HTMLDivElement | null>(null);
	...
	// Virtualization: Group messages into chunks of 20
	const messageChunks = $derived.by(() => {
		const filtered = chatStore.messages.filter((m) => !m.parent_id);
		const chunkSize = 20;
...
<div class="flex h-full bg-surface-dim/20">
	{#if !$uiStore.isMobile || !chatStore.selectedThreadId}
		<div class="flex min-w-0 flex-1 flex-col">
				...
				{:else if chatStore.messages.length === 0}
					<div class="flex flex-1 flex-col items-center justify-center p-12 text-center">
						...
						<h3 class="text-lg font-bold text-content italic">
							Welcome to {chatStore.activeChannel?.type === 'text' ? '#' : ''}{chatStore
								.activeChannel?.display_name}!
						</h3>
						<p class="mt-2 max-w-xs text-sm text-content-dim">
							This is the start of your conversation in {chatStore.activeChannel?.display_name}.
							Send a message to get things rolling.
						</p>
					</div>
				{:else}
					{#each messageChunks as chunk, i (i)}
						<MessageChunk
							messages={chunk}
			...
			<footer class="shrink-0 bg-transparent p-4 {$uiStore.isMobile ? 'pb-20' : ''}">
	...
	{#if chatStore.selectedThreadId}
		<div transition:slide={{ axis: 'x', duration: 400 }} class="h-full flex-1">
			<ThreadView
